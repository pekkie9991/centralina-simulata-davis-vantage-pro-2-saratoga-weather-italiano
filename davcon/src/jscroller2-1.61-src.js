...
var ByRei_jScroller2 = {
 ...
 /* 
   ...
   [3 / option].direction:[Text]       - set the direction (up,down,left,right)
   [3 / option].speed:[Number]         - set the speed of the scrolling
   [3 / option].pause:[Boolean]        - pause the scroller so it will not start
   [3 / option].delay:[Number]         - delay the start of the scroller in sec.
 ...
 get: {
  ...
  /* Complex Checking for Endless DIV return and set position */
 ...
 /* 
  This includes all on(action) Methods 
 */
 on: {
 ...
 */
 set: {
  /* Simple Wrapper to include "px" on every value */
 ...
 */
 init: {
  ...
  main: function() {
   ...
   /* Check if some Objects have a direction */
   for (i=0;i<il;i++) {
    var 
 ...
 add: function(obj, direction) {
  ...
  /* Check for duplication, when Object is existing the direction will be replaced nothing more to do... */
  if (obj && direction) {  
      if (il > 0) {
          for (i=0;i<il;i++) {
 ...
 /* Remove jScroller2 Object from the List complete */
 remove: function(obj) {
  if (obj) {
     for (var i=0;i<ByRei_jScroller2.obj.length;i++) {
         if (ByRei_jScroller2.obj[i][1].obj === obj) {
 ...
 /* Main Part - Scroller Events which will execute for a special Interval */
 scroller: function() {
  var
   i,
   il = ByRei_jScroller2.obj.length;
...
/* Init the whole jScroller2 after the Document loading completed */
