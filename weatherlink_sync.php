<?php
// ABILITA LA DIAGNOSTICA DEI PARSE ERROR (Sicura e attiva)
...
if (!function_exists('translate_davis_forecast')) {
    function translate_davis_forecast($forecast) {
        ...
        $dictionary = array(
            ...
            // 4. Condizioni di miglioramento / Schiarite / Neve, Temporali, Grandine e Termiche
            'clearing, cooler and windy' => 'Schiarite, più fresco e ventoso',
            'clearing and cooler' => 'Schiarite e più fresco',
            'clearing' => 'Schiarite',
            'snow possible' => 'Neve possibile',
            'rain and snow possible' => 'Possibile pioggia e neve',
            'rain and/or snow possible' => 'Possibile pioggia e/o neve',
            'heavy thunderstorm with possible hail and high winds' => 'Forte temporale con possibile grandine e forti venti',
            'heavy thunderstorm possible' => 'Possibile forte temporale',
            'misty or foggy conditions possible' => 'Possibile nebbia o foschia',
            'freezing conditions possible' => 'Possibile gelata o brina',
            'extreme wind chill' => 'Freddo estremo da vento gelido',
            'extreme heat index' => 'Afa opprimente e calore estremo',
            'high winds. gale warning' => 'Forti venti. Allerta burrasca',
            ...
            'occasional precipitation, worsening' => 'Precipitazioni occasionale, in peggioramento',
...
if ($fetch_needed) {
    ...
    if ($raw) {
        ...
        if ($data && isset($data['davis_current_observation'])) {
            ...
            // Generazione predittiva dinamica ad alta fedeltà basata sul calcolo barometrico Davis (Davis Forecast Engine v6 - Convective)
            if (empty($wsforecast)) {
                $baro_trend_str = $davis['pressure_tendency_string'] ?? 'Steady';
                $hum_out = isset($data['relative_humidity']) ? (int)$data['relative_humidity'] : 50;
                $temp_out = isset($data['temp_c']) ? (float)$data['temp_c'] : 20.0;
                $wind_avg_kmh = isset($davis['wind_ten_min_avg_mph']) ? (float)$davis['wind_ten_min_avg_mph'] * 1.60934 : 0.0;
                $wind_gust_kmh = isset($davis['wind_ten_min_gust_mph']) ? (float)$davis['wind_ten_min_gust_mph'] * 1.60934 : 0.0;
                $daily_rain_mm = isset($davis['rain_day_in']) ? (float)$davis['rain_day_in'] * 25.4 : 0.0;
                $rain_rate_mmh = isset($davis['rain_rate_in_per_hr']) ? (float)$davis['rain_rate_in_per_hr'] * 25.4 : 0.0;
                $windchill_c = isset($data['windchill_c']) ? (float)$data['windchill_c'] : $temp_out;
                $heat_index_c = isset($data['heat_index_c']) ? (float)$data['heat_index_c'] : $temp_out;

                // 1. Condizioni del cielo in base alla pressione, all'umidità e alla pioggia (Schiarite)
                if (stripos($baro_trend_str, 'Rising Rapidly') !== false) {
                    if ($daily_rain_mm > 0 && $rain_rate_mmh == 0) {
                        $wsforecast = 'Clearing and cooler.';
                    } else {
                    ...
                    if ($hum_out < 50) {
                        ...
                        $wsforecast = 'Mostly cloudy and cooler.';
                    }
                } else { // Steady / Stabile
                    $wsforecast = 'Partly cloudy with little temperature change.';
                }

                // 2. Probabilità di precipitazione legate alla tendenza barometrica (Inverno: neve, Estate: temporali, grandine)
                if (stripos($baro_trend_str, 'Falling Rapidly') !== false) {
                    if ($hum_out >= 50) {
                        if ($temp_out <= 3.0) {
                            $wsforecast .= ' Snow possible.';
                        } elseif ($temp_out <= 6.0) {
                            $wsforecast .= ' Rain and snow possible.';
                        } else {
                            // Condizioni fisiche per il calcolo del rischio Grandine (Cella temporalesca estiva convettiva)
                            if ($temp_out >= 24.0 && $hum_out >= 75 && ($wind_gust_kmh >= 35.0 || $rain_rate_mmh >= 10.0)) {
                                $wsforecast .= ' Precipitation likely. Heavy thunderstorm with possible hail and high winds.';
                            } elseif ($hum_out >= 80 && $temp_out >= 20.0) {
                                $wsforecast .= ' Precipitation likely. Heavy thunderstorm possible.';
                            } else {
                    ...
                    if ($hum_out >= 55) {
                        if ($temp_out <= 3.0) {
                            ...
                            $wsforecast .= ' Rain and/or snow possible.';
                        } else {
                            $wsforecast .= ' Precipitation possible.';
                        }
                    }
                ...
                // 3. Aggiunte predittive sul vento, foschia e allerte termiche
                if (stripos($baro_trend_str, 'Falling Rapidly') !== false && $hum_out >= 50) {
                    if ($wind_avg_kmh >= 15.0 || $wind_gust_kmh >= 25.0) {
                        $wsforecast .= ' Precipitation possible and windy within 6 hours.';
                    } else {
                        $wsforecast .= ' Precipitation possible within 6 hours.';
                    }
                } elseif (stripos($baro_trend_str, 'Falling Slowly') !== false && $hum_out >= 55) {
                    $wsforecast .= ' Precipitation possible within 6 hours.';
                } elseif (stripos($baro_trend_str, 'Rising Rapidly') !== false) {
                    if ($wind_avg_kmh >= 15.0 || $wind_gust_kmh >= 25.0) {
                        $wsforecast .= ' Windy.';
                    }
                }

                // Allerta burrasca / vento forte se le raffiche superano i 40 km/h
                if ($wind_gust_kmh >= 40.0) {
                    $wsforecast .= ' High winds. Gale warning.';
                }

                // Aggiunta nebbia/foschia se umidità altissima e vento calmo (Steady/Rising Slowly)
                if ($hum_out >= 90 && $wind_avg_kmh < 5.0 && stripos($baro_trend_str, 'Falling') === false) {
                    $wsforecast .= ' Misty or foggy conditions possible.';
                }

                // Aggiunta avviso gelo/brina se temperatura sotto lo zero
                if ($temp_out <= 0.0) {
                    $wsforecast .= ' Freezing conditions possible.';
                }

                // Allerta freddo estremo da vento (Wind Chill sottozero)
                if ($windchill_c <= 0.0 && $temp_out > 0.0) {
                    $wsforecast .= ' Extreme wind chill.';
                }

                // Allerta afa opprimente (Heat Index molto alto)
                if ($heat_index_c >= 35.0) {
                    $wsforecast .= ' Extreme heat index.';
                }
            }
...
if (file_exists($dataurl)) {
    ...
    if (count($realtime_data) >= 79) {
        ...
        if (!empty($realtime_data[99])) {
            ...
            // Mappatura simulata ad alta fedeltà se il file di cache non è ancora aggiornato (Davis Forecast Engine v6 - Convective)
            ...
            $hum_out = isset($realtime_data[3]) ? (int)$realtime_data[3] : 50;
            $temp_out = isset($realtime_data[2]) ? (float)$realtime_data[2] : 20.0;
            $daily_rain_mm = isset($realtime_data[9]) ? (float)$realtime_data[9] : 0.0;
            ...
            if (stripos($baro_tendency, 'Rising Rapidly') !== false) {
                ...
                if ($hum_out < 50) {
                    $wsforecast = 'Mostly cloudy with little temperature change. Precipitation possible.';
                } else {
                    if ($temp_out <= 3.0) {
                        $wsforecast = 'Mostly cloudy and cooler. Snow possible. Precipitation possible within 6 hours.';
                    } elseif ($temp_out <= 6.0) {
                        $wsforecast = 'Mostly cloudy and cooler. Rain and snow possible. Precipitation possible within 6 hours.';
                    } else {
                        if ($hum_out >= 80 && $temp_out >= 20.0) {
                            $wsforecast = 'Mostly cloudy with little temperature change. Precipitation likely. Heavy thunderstorm possible. Precipitation possible and windy within 6 hours.';
                        } else {
                ...
                if ($hum_out < 55) {
                    ...
                    if ($temp_out <= 3.0) {
                        $wsforecast = 'Mostly cloudy with little temperature change. Snow possible. Precipitation possible within 6 hours.';
                    } elseif ($temp_out <= 6.0) {
                        $wsforecast = 'Mostly cloudy with little temperature change. Rain and/or snow possible. Precipitation possible within 6 hours.';
                    } else {
                        $wsforecast = 'Mostly cloudy with little temperature change. Precipitation possible. Precipitation possible within 6 hours.';
                    }
                }
            } else {
                $wsforecast = 'Partly cloudy with little temperature change.';
